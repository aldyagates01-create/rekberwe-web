...
export function TransactionSummaryCollapse({
  ...
  return (
    <section className="shrink-0 bg-bg px-2 py-1">
      <div className="rounded-xl border border-border bg-card/95 shadow-[0_16px_40px_rgba(0,0,0,0.18)]">
        <div className="grid grid-cols-2 gap-2 border-b border-border px-2.5 py-1.5">
          <div className="min-w-0">
            <p className="text-[9px] text-white/45">Status Transaksi</p>
            <p className="inline-flex max-w-full truncate rounded-md bg-accent-blue/15 px-1.5 py-0.5 text-[8px] font-bold uppercase tracking-wide text-accent-blue">
              {shortStatus}
            </p>
          </div>
          <div className="min-w-0 text-right">
            <p className="text-[9px] text-white/45">Nilai Transaksi</p>
            <p className="truncate text-[13px] font-bold text-success">{formatCurrency(transaction.price)}</p>
          </div>
        </div>
          ...
          {steps.map((step, index) => {
            ...
            return (
              <div key={step.label} className="relative flex min-w-0 flex-col items-center gap-1 text-center">
                {index > 0 ? (
                  <span
                    className={`absolute right-1/2 top-3 h-px w-full -translate-x-4 ${
                ...
                <span
                  className={`relative z-10 inline-flex h-6 w-6 items-center justify-center rounded-lg text-[9px] ring-1 ${
                    active
                ...
                <span className="max-w-[58px] text-[8px] font-medium leading-tight text-white/60">{step.label}</span>
              </div>
            );
          })}
        </div>
  ...
  return (
    <div className="flex min-w-0 flex-col items-center px-1 text-center">
      ...
      {badge ? (
        ...
        <span className="mt-0.5 flex items-center gap-1 text-[8px] text-success">
