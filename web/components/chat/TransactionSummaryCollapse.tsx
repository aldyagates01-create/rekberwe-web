...
export function TransactionSummaryCollapse({
  ...
  return (
    <section className="shrink-0 space-y-2 bg-bg px-3 pb-2 pt-2">
      <div className="rounded-2xl border border-border bg-card/95 shadow-[0_16px_40px_rgba(0,0,0,0.18)]">
        <div className="grid grid-cols-2 gap-3 border-b border-border px-4 py-3">
          <div className="min-w-0">
            <p className="text-[11px] text-white/45">Status Transaksi</p>
            <p className="mt-1 inline-flex max-w-full truncate rounded-lg bg-accent-blue/15 px-2 py-1 text-[10px] font-bold uppercase tracking-wide text-accent-blue">
              {shortStatus}
            </p>
          </div>
          <div className="min-w-0 text-right">
            <p className="text-[11px] text-white/45">Nilai Transaksi</p>
            <p className="mt-1 truncate text-base font-bold text-success">{formatCurrency(transaction.price)}</p>
          </div>
        </div>

        <div className="grid grid-cols-3 divide-x divide-border border-b border-border px-2 py-3">
          ...
          {steps.map((step, index) => {
            ...
            return (
              <div key={step.label} className="relative flex min-w-0 flex-col items-center gap-1 text-center">
                {index > 0 ? (
                  <span
                    className={`absolute right-1/2 top-4 h-px w-full -translate-x-5 ${
                ...
                <span
                  className={`relative z-10 inline-flex h-8 w-8 items-center justify-center rounded-xl text-xs ring-1 ${
                    active
                ...
                <span className="max-w-[64px] text-[10px] font-medium leading-tight text-white/60">{step.label}</span>
              </div>
            );
          })}
        </div>
      ...
      <div className="flex gap-2 rounded-2xl border border-accent-blue/15 bg-accent-blue/10 px-3 py-2">
        <span className="mt-0.5 inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-accent-blue/20 text-[11px] font-bold text-accent-blue">
          ✓
        </span>
...
function Participant({
  ...
  return (
    <div className="flex min-w-0 flex-col items-center px-1 text-center">
      <div className={`mb-1 inline-flex h-9 w-9 items-center justify-center overflow-hidden rounded-full text-xs font-bold ${accentClass}`}>
        {user?.avatar ? (
      ...
      <strong className="max-w-full truncate text-[11px] text-white">{name}</strong>
      {badge ? (
        <span className="mt-0.5 rounded-full bg-accent-purple/15 px-1.5 py-0.5 text-[9px] font-bold text-accent-purple">
          {badge}
        </span>
      ) : (
